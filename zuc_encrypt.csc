...
while system.in.good()
    ...
    system.out.println("ZUC IV: " + gmssl.bytes_decode(gmssl.hex_encode(iv)))

    var encrypted = gmssl.zuc_encrypt(key, iv, gmssl.base64_encode(input_bytes))
    system.out.println("ZUC encrypted: " + gmssl.bytes_decode(gmssl.hex_encode(encrypted)))

    var decrypted = gmssl.base64_decode(gmssl.zuc_encrypt(key, iv, encrypted))
    system.out.println("ZUC decrypted: " + gmssl.bytes_decode(decrypted))
end
