...
int main()
{
	...
	while (std::getline(std::cin, input)) {
		...
		gmssl::uint8_array_t ctr_encrypted = gmssl::sm4(gmssl::sm4_mode::ctr_encrypt, key, iv, gmssl::base64_encode(input_bytes));
		std::cout << "SM4 CTR encrypted: " << gmssl::bytes_decode(gmssl::hex_encode(ctr_encrypted)) << std::endl;
		std::cout << "SM4 CTR decrypted: " << gmssl::bytes_decode(gmssl::base64_decode(gmssl::sm4(gmssl::sm4_mode::ctr_decrypt, key, iv, ctr_encrypted))) << std::endl;
